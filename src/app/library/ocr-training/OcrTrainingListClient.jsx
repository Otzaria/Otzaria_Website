...
// חלק האינטראקציה של דף רשימת עמודי אימון ה-OCR: הרשימה הראשונית מגיעה
// כ-prop מה-Server Component (page.jsx) — אין כאן fetch בעת העלייה. תפיסת
// עמוד (handleClaim) עדיין קריאת API רגילה, ולאחריה ניווט לדף הסימון.
export default function OcrTrainingListClient({ initialPages, loadError }) {
  ...
  useEffect(() => {
    if (loadError) showAlert('שגיאה', 'שגיאה בטעינת רשימת העמודים')
