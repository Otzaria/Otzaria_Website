...
// כל האינטראקטיביות של דף ניהול ההודעות. רשימת ההודעות הראשונית מגיעה
// כ-prop מה-Server Component (page.jsx, נשלפת ממטמון ה-Data Cache) —
...
export default function AdminMessagesClient({ initialMessages }) {
  ...
  // מיון לפי זמן הפעילות האחרונה (תגובה אחרונה, אחרת יצירה) — יורד. אותו
  // מיון בדיוק שהיה קודם ב-loadData; מופעל גם על הרשימה הראשונית מהשרת כדי
  // שהסדר יהיה זהה גם בטעינה הראשונה וגם אחרי רענון.
  ...
  const loadUsersBasic = async () => {
    try {
      const usersRes = await fetch('/api/admin/users-basic')
