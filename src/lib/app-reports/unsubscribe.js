...
const sign = (reportId, secret) => createHmac('sha256', secret).update(`app-reports-unsubscribe:${reportId}`).digest('base64url');

export function createUnsubscribeToken(reportId, secret) {
  if (!secret) throw new Error('unsubscribe secret missing');
  return `${Buffer.from(reportId, 'utf8').toString('base64url')}.${sign(reportId, secret)}`;
}
...
export function verifyUnsubscribeToken(token, secret) {
  ...
  const expected = Buffer.from(sign(reportId, secret));
